...
def run_pipeline(source, language_input):
    ...
    chunks = [] # Initialize chunks list so the finally block can access it safely
    ...
    try:
        ...
        # --- NEW: Cloud Disk Space Cleanup ---
        # Delete audio chunks generated by pydub
        for chunk in chunks:
        ...
        # If the user uploaded a file, delete the temporary file from the server
        if os.path.exists(source) and "http" not in source:
            try:
                os.remove(source)
            except:
                pass
